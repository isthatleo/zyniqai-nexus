...
import { MessageCircle, X, Send, User, Minimize2, Volume2, VolumeX } from "lucide-react";
...
const AIChatWidget = () => {
  ...
  useEffect(() => {
    if (open && leadCaptured && messages.length === 0) {
      ...
      if (voiceEnabled) speakText(welcomeMsg);
    }
  ...
  const handleLeadSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await fetch(SAVE_URL, {
        ...
        body: JSON.stringify({
          lead_data: { name: leadInfo.name, email: leadInfo.email, company: leadInfo.company || null, status: "new" },
  ...
  const send = async () => {
    ...
    try {
      ...
      const finalCleaned = cleanMessage(assistantSoFar);
      if (voiceEnabled && finalCleaned) speakText(finalCleaned);
      ...
      if (leadData) {
        fetch(SAVE_URL, {
          ...
          body: JSON.stringify({
            lead_data: { ...leadData, name: leadInfo.name, email: leadInfo.email, company: leadInfo.company || leadData.company || null },
            session_id: sessionId,
            messages: [userMsg, { role: "assistant", content: finalCleaned }],
          }),
        }).catch(console.error);
      }
    } catch (e) {
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-border/30 bg-card/50">
              <div className="flex items-center gap-2.5">
                <div className="relative">
                  ...
                  {isSpeaking && (
                    <span className="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full bg-primary animate-pulse border-2 border-background" />
                  )}
                </div>
                <div>
                  <p className="text-sm font-semibold">ZyniqAI Reception</p>
                  <p className="text-[10px] text-muted-foreground">
                    {isSpeaking ? "Speaking..." : "Voice-First AI Gatekeeper"}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-1">
                <button
                  ...
                  className={`w-7 h-7 rounded-full flex items-center justify-center transition-all ${voiceEnabled ? "text-primary bg-primary/10" : "text-muted-foreground hover:text-foreground hover:bg-muted/50"}`}
                ...
                <button onClick={() => { stopSpeaking(); setOpen(false); setMessages([]); setLeadCaptured(false); setLeadInfo({ name: "", email: "", company: "" }); }} className="w-7 h-7 rounded-full flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-muted/50 transition-all">
                  <X size={14} />
                </button>
              </div>
            </div>
            ...
            {!leadCaptured ? (
              <div className="flex-1 flex flex-col items-center justify-center px-6 py-8">
                ...
                <p className="text-xs text-muted-foreground text-center mb-6 max-w-[260px]">
                  Our voice-first AI consultant will speak with you directly. Please provide a few details to begin.
                </p>
                <form onSubmit={handleLeadSubmit} className="w-full space-y-3">
                  <input type="text" placeholder="Full Name *" value={leadInfo.name} onChange={(e) => setLeadInfo(p => ({ ...p, name: e.target.value }))} required
                  ...
                  <input type="email" placeholder="Email Address *" value={leadInfo.email} onChange={(e) => setLeadInfo(p => ({ ...p, email: e.target.value }))} required
                    className="w-full px-3.5 py-2.5 rounded-xl bg-muted/40 border border-border/40 text-sm text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:border-primary/50 transition-all" />
                  <input type="text" placeholder="Company Name" value={leadInfo.company} onChange={(e) => setLeadInfo(p => ({ ...p, company: e.target.value }))}
                    className="w-full px-3.5 py-2.5 rounded-xl bg-muted/40 border border-border/40 text-sm text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:border-primary/50 transition-all" />
                  <button type="submit" className="w-full py-2.5 rounded-xl bg-primary text-primary-foreground text-sm font-medium hover:bg-primary/90 transition-all">
              ...
              <>
                  ...
                  {messages.map((msg, i) => (
                    ...
                    >
                      ...
                      <div className={`max-w-[80%] px-3.5 py-2.5 rounded-2xl text-[13px] leading-relaxed ${
                        ...
                        {msg.role === "assistant" ? (
                          <div className="prose prose-sm prose-invert max-w-none [&>p]:m-0 [&>p+p]:mt-2 [&>ul]:my-1 [&>ol]:my-1 [&_li]:text-[13px]">
                ...
                {/* Voice indicator */}
                {isSpeaking && (
                  <div className="px-4 py-1.5 bg-primary/5 border-t border-primary/10 flex items-center justify-center gap-2">
                    ...
                    <button onClick={stopSpeaking} className="text-[10px] text-muted-foreground hover:text-foreground ml-1">Skip</button>
                  </div>
                )}
                ...
                <div className="px-4 py-3 border-t border-border/30 bg-card/30">
                  ...
                  <p className="text-[9px] text-muted-foreground/50 text-center mt-2">Powered by ZyniqAI Intelligence · Voice by ElevenLabs</p>
                </div>
              </>
            )}
          </motion.div>
