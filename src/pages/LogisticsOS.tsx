          ...
          <div className="max-w-6xl mx-auto">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-16">
              ...
              <div className="grid md:grid-cols-3 gap-4">
                {[
                  { status: "En Route", count: 87, color: "neon-green" },
                  { status: "At Warehouse", count: 38, color: "neon-blue" },
                  { status: "Maintenance", count: 17, color: "neon-purple" },
                ].map((s) => (
                  <div key={s.status} className="glass-card p-5">
                    <div className="flex items-center justify-between mb-3">
                      <p className="text-sm font-medium">{s.status}</p>
                      <span className={`text-xs font-bold text-${s.color}`}>{s.count}</span>
                    </div>
                    <div className="w-full h-2 rounded-full bg-muted/50">
                      <div className={`h-full rounded-full bg-${s.color}`} style={{ width: `${(s.count / 142) * 100}%` }} />
                    </div>
                  </div>
                ))}
              </div>
